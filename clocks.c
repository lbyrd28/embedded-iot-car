...
// -----------------------------------------------------------------------------
  ...
  CSCTL2 = FLLD_0 + FREQADD;     // DCODIV = 8MHz
...
// All rights reserved.
  unsigned int oldDcoTap = ONE_FILLS;
  unsigned int newDcoTap = ONE_FILLS;
  unsigned int newDcoDelta = ONE_FILLS;
  unsigned int bestDcoDelta = ONE_FILLS;
  ...
  unsigned int dcoFreqTrim = FREQ_TRIM;
  ...
  do{
    ...
    __delay_cycles((unsigned int)FREQMULT * MCLK_FREQ_MHZ);
    while((CSCTL7 & (FLLUNLOCK0 | FLLUNLOCK1)) &&
         ((CSCTL7 & DCOFFG) == FALSE));
    csCtl0Read = CSCTL0;                    // Read CSCTL0
    csCtl1Read = CSCTL1;                    // Read CSCTL1
    oldDcoTap = newDcoTap;                  // Record DCOTAP value of last time
    newDcoTap = csCtl0Read & NINE_BIT_TRUE;        // Get DCOTAP value of this time
    dcoFreqTrim = (csCtl1Read & CUT_OFF_LAST_4)>>FREQ_CUTOFF; // Get DCOFTRIM value
    if(newDcoTap < DCO_TAP){                    // DCOTAP < 256
      newDcoDelta = DCO_TAP - newDcoTap;        // Delta value between DCPTAP and 256
      if((oldDcoTap != ONE_FILLS) &&
         (oldDcoTap >= DCO_TAP)){               // DCOTAP cross 256
        endLoop = TRUE;                        // Stop while loop
      }else{
        dcoFreqTrim--;
        CSCTL1 = (csCtl1Read & (~DCOFTRIM)) | (dcoFreqTrim<<FREQ_CUTOFF);
      }
    }else{                                  // DCOTAP >= 256
      newDcoDelta = newDcoTap - DCO_TAP;        // Delta value between DCPTAP and 256
      if(oldDcoTap < DCO_TAP){                  // DCOTAP cross 256
        endLoop = TRUE;                        // Stop while loop
      }else{
        dcoFreqTrim++;
        CSCTL1 = (csCtl1Read & (~DCOFTRIM)) | (dcoFreqTrim<<FREQ_CUTOFF);
